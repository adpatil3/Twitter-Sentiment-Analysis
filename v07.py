...
def clean(df, name):
	...
	df['polarity'] = df['tidy_tweet'].apply(lambda x: polarity_bin(tb(x).sentiment.polarity))
	df['Name'] = name
	return df
...
def piechart(df1,df2):
	...
	axs[0].pie(df1.Tweet.groupby(df1['polarity']).count(), labels=labels, explode=explode, 
	   textprops={'fontsize': 14}, autopct='%1.1F%%')
	axs[0].set_title(df1['Name'][0],fontsize = 20)
	axs[0].set_xlabel('Total Tweets: {}'.format(df1['polarity'].count()),fontsize = 14)
	axs[1].pie(df2.Tweet.groupby(df2['polarity']).count(), labels=labels, explode=explode, 
	   textprops={'fontsize': 14}, autopct='%1.1F%%')
	axs[1].set_title(df2['Name'][0],fontsize = 20)
	axs[1].set_xlabel('Total Tweets: {}'.format(df2['polarity'].count()),fontsize = 14)
	return plt.show()
...
c = pd.crosstab(index=df['polarity'],columns=df['Name'],rownames=['Sentiment'])
print(c)
...
df_train = df_train[['tidy_tweet','polarity']]
df_test = df_test[['tidy_tweet','polarity']]
...
# Evaluating Results:
# Confusion Matrix:
con_mat = cm(df_test['polarity'], df_test['Predicted'])

from pandas_ml import ConfusionMatrix
confusion_matrix = ConfusionMatrix(df_test['polarity'], df_test['Predicted'])
...
X_train, X_test, y_train, y_test = train_test_split(df_c['cleaned'], df_c['polarity'], 
